# Writing our tables from the three different data files. This module does a quick check on the database to see if the tables already exist.
# If they don't, it writes a new one for each file and adds an index to them to help speed up merges if the analysis is going to be run multiple times.
# If a table does exist, it just passes over it and moves on to the next one.
